...
Provide a password-gated iPhone and iPad onboarding flow that collects a device UDID through Apple's OTA Profile Service protocol, registers the device in the Apple Developer team, rebuilds the Ad Hoc IPA, and presents the OTA install action without exposing the UDID in public GitHub inputs or logs.

## User Experience

1. The user opens the Asspp registration page in Safari.
2. The page asks: `作者的微信号 ID 是多少？`
3. A correct server-side answer unlocks one onboarding request for 60 minutes.
...
The first onboarding normally takes approximately 7 minutes, with additional delay possible while GitHub Pages is queued. A previously registered device can use the latest OTA page without consuming another device slot.
...
- Create a cryptographically random, single-use challenge with a 60-minute expiry.
...
- Delete raw CMS and UDID-bearing material after completion or expiry, then delete request metadata 24 hours after expiry.
...
6. Create and download a fresh Ad Hoc profile containing all enabled iPhone, iPad, and iPod devices for the Asspp bundle ID and active distribution certificate.
...
- `state`: `unlocked`, `device_received`, `building`, `ready`, `failed`, or `expired`
...
The Worker deletes `cms_payload` after the GitHub runner claims it. A 15-minute cron clears any unclaimed CMS when the request expires, and deletes request metadata 24 hours after expiry.
...
4. A real iPhone or iPad completes the Safari profile flow.
